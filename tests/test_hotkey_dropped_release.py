...
class HandsfreeDoubleTapDroppedReleaseStops(unittest.TestCase):
    """The REAL stuck-mic hit on v1.0.9: a hands-free double-tap whose SECOND
    tap's key-UPs are dropped by macOS never reaches the latch (which happens on
    release) — the FSM is left mid-"hold" with recording ON and the chord believed
    down. Pressing the chord again to stop must STOP, not start a new recording."""

    def test_double_tap_with_dropped_second_release_can_be_stopped(self):
        hl, ev, rec = make(mode="push_to_talk", handsfree=True, double=0.4)
        # tap 1, releases delivered -> tiny clip started then discarded
        hl._feed_press(CTRL)
        hl._feed_press(ALT)
        hl._feed_release(ALT)
        hl._feed_release(CTRL)
        # tap 2 (quick), but BOTH key-UPs dropped -> never latches, wedged ON
        hl._feed_press(CTRL)
        hl._feed_press(ALT)
        self.assertTrue(rec["on"])           # recording, but wedged
        # user presses the chord again to STOP
        hl._feed_press(CTRL)
        hl._feed_press(ALT)
        self.assertFalse(rec["on"])          # mic OFF — the user CAN stop
        self.assertFalse(hl._latched)

    def test_dropped_release_during_plain_hold_can_be_stopped(self):
        """Push-to-talk hold whose release is dropped: a re-press must stop, not
        begin a second recording."""
        hl, ev, rec = make(mode="push_to_talk", handsfree=True, double=0.4)
        hl._feed_press(CTRL)
        hl._feed_press(ALT)
        # hold release dropped -> recording stuck on
        self.assertTrue(rec["on"])
        hl._feed_press(CTRL)                 # re-press to stop
        hl._feed_press(ALT)
        self.assertFalse(rec["on"])

    def test_normal_double_tap_still_latches_then_stops(self):
        """Regression guard: the clean double-tap path is unchanged."""
        hl, ev, rec = make(mode="push_to_talk", handsfree=True, double=0.4)
        hl._feed_press(CTRL)
        hl._feed_press(ALT)
        hl._feed_release(ALT)
        ...
        hl._feed_release(ALT)
        hl._feed_release(CTRL)
        self.assertTrue(rec["on"])           # latched hands-free
        self.assertTrue(hl._latched)
        hl._feed_press(CTRL)                 # press again to stop
        hl._feed_press(ALT)
        self.assertFalse(rec["on"])
